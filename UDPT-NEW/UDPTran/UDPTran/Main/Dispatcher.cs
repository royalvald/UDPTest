...
namespace UDPTran
{
    ...
    class Dispatcher
    {
        ...
        public Dispatcher(string IP, int port)
        {
            ...
            IPAddress selfAddress = IPAddress.Parse("192.168.109.22");
            hostIPEndPoint = new IPEndPoint(selfAddress, 8090);
            listener = new TcpListener(new IPEndPoint(selfAddress, 8000));
            //接收池与发送池初始化
        ...
        private void Service(object objects)
        {
            ...
            while (client.Connected)
            {
                ...
                int size = stream.Read(tcpBytes, 0, 20);
                if (size > 1)
                    tcpBytes.Initialize();
                break;
            }
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    infoByte = new byte[1040];
                    Array.Copy(TempInfo, 0, infoByte, 0, 1040);
                    byteList.AddRange(infoByte);
                    if (byteList.Count > 5 * 1024 * 1024)
                    {
        ...
        private void ByteToFile(byte[] bytes)
        {
            if (File.Exists("./test"))
            {
                while (true)
                {
                    ...
                    {
                        fileLock = false;
                        FileStream fs = File.Open("./test", FileMode.Append, FileAccess.Write);
            ...
            else
            {
                while (true)
                {
                    ...
                    {
                        fileLock = false;
                        FileStream fs = File.Open("./test", FileMode.Create, FileAccess.Write);
                        fs.Write(bytes, 0, bytes.Length);
                        fs.Close();
                        fileLock = true;
                    }
        ...
        private int[] FileCheck(ref int count)
        {
            ...
            if (File.Exists("./test"))
            {
                ...
                while (position < fs.Length)
                {
                    fs.Position = position;
                    fs.Read(bytes, 0, 4);
                    list.Add(BitConverter.ToInt32(bytes, 0));
                    position += 1040;
                }
                return list.ToArray();
            }
